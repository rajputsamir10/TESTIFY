...
function ExamInterface() {
  ...
  }, [redirectScheduled, router])

  const handleViolationAutoSubmit = useCallback((reason) => {
    if (reason === 'camera_violation') {
      setLockOverlayMessage('Exam auto-submitted due to repeated proctoring violations.')
    } else {
  ...
  const { cameraStatus, violationCount: cameraViolations } = useWebcamProctoring(attemptId, handleViolationAutoSubmit, Boolean(attemptId))
  const violationCount = Math.max(violations, cameraViolations)
  ...
  useEffect(() => {
    ...
    void bootstrap()
  }, [attemptId, examId, hydrate, router])
    ...
    <div className="relative space-y-4">
      ...
      <div className="card flex flex-wrap items-center justify-between gap-2 p-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="chip">Violations: {violationCount}</span>
        </div>
      </div>
